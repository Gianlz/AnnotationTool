...
- **Secondary**: Transparent background, Contrast Border (Carbon Black `#111111` in Light, Nebula White `#FFFFFF` in Dark) (`1px`).

### Visual Style

- **Shadows**: Minimal or absent. Rely on high contrast boundaries.
- **Borders**: Thin, crisp lines. Nebula White (`#FFFFFF`) or Steel Grey (`#333333`) for subtle separation.
- **Spacing**: Architectural and airy. Avoid clutter.
- **Surfaces**: Use Steel Grey (`#333333`) for elevated areas (cards/nav) to distinguish from the Carbon Black base in Dark Mode.
...
- **Render Scope**: Components must **only** trigger updates in related components that genuinely require re-rendering. **Mandatory Check**: Verify render behavior after _every_ implementation to prevent performance regression.
